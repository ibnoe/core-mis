...
class Regpyd extends Front_Controller{
	
	private $per_page 	= '10';
	private $title 		= 'Regpyd';
	private $module 	= 'regpyd';
	...
	public function download(){
		if($this->session->userdata('logged_in'))
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue("X4", "NO KTP");
			$objPHPExcel->getActiveSheet()->setCellValue("Y4", "PPI (Nilai)");
			$objPHPExcel->getActiveSheet()->setCellValue("Z4", "PPI (Kategori)");
			...
			foreach($regpyd as $c): 
				...
				if($c->client_birthdate AND $c->client_ktp AND $p->data_popi_anggotart AND $p->data_popi_masihsekolah AND $p->data_popi_pendidikanistri AND $p->data_popi_pekerjaansuami AND $p->data_popi_jenislantai AND $p->data_popi_jeniswc AND $p-> data_popi_bahanbakar AND $p->data_popi_gas AND $p-> data_popi_kulkas AND $p->data_popi_motor AND $p->data_popi_total AND $p->data_popi_kategori AND $p->data_rmc_ukuranrumah AND $p->data_rmc_kondisirumah AND $p->data_rmc_jenisatap AND $p->data_rmc_jenisdinding AND $p->data_rmc_jenislantai AND $p->data_rmc_listrik AND $p->data_rmc_sumberair AND $p->data_rmc_total AND $p->data_rmc_kategori)
				{
					$database= "OK"; 
				}else{ 
					$database= "NOK";
				...
				$objPHPExcel->getActiveSheet()->setCellValue("T$cell", $p->data_tujuan);
				...
				$objPHPExcel->getActiveSheet()->setCellValue("X$cell", $c->client_ktp);
				$objPHPExcel->getActiveSheet()->setCellValue("Y$cell", $p->data_popi_total);
				$objPHPExcel->getActiveSheet()->setCellValue("Z$cell", $p->data_popi_kategori);
	...
	public function tr_log_download(){
		if($this->session->userdata('logged_in'))
		{
			//Cek User Login Branch
			$user_branch = $this->session->userdata('user_branch');
			
			$regpyd = $this->regpyd_model->get_all_transaction('2015-05-01','2015-05-31');	 
			...
			$objPHPExcel->getProperties()->setCreator("Amartha MIS");
			$objPHPExcel->getProperties()->setLastModifiedBy("Amartha MIS");
			$objPHPExcel->getProperties()->setTitle("Log Transaksi");
			$objPHPExcel->getProperties()->setSubject("Log Transaksi");
			$objPHPExcel->getProperties()->setDescription("Log Transaksi");
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Log Transaksi');
			
			//TITLE
			$objPHPExcel->getActiveSheet()->setCellValue("A1", "Amartha Microfinance");
			$objPHPExcel->getActiveSheet()->setCellValue("A2", "Log Transaksi");
			$objPHPExcel->getActiveSheet()->mergeCells("A1:L1");
			$objPHPExcel->getActiveSheet()->mergeCells("A2:L2");
			$objPHPExcel->getActiveSheet()->getStyle("A1")->applyFromArray(array("font" => array( "bold" => true, 'size'  => 16)));
			$objPHPExcel->getActiveSheet()->getStyle("A2")->applyFromArray(array("font" => array( "bold" => true)));
			...
			foreach($regpyd as $c): 
				...
				$objPHPExcel->getActiveSheet()->setCellValue("H$cell", $p->tr_angsuranpokok);
				$objPHPExcel->getActiveSheet()->setCellValue("I$cell", $p->tr_profit);
				$objPHPExcel->getActiveSheet()->setCellValue("J$cell", $p->data_date_accept);
				$objPHPExcel->getActiveSheet()->setCellValue("K$cell", $p->data_jatuhtempo);
				$objPHPExcel->getActiveSheet()->setCellValue("L$cell", $p->data_ke);
				$objPHPExcel->getActiveSheet()->setCellValue("M$cell", $p->data_angsuranke);
