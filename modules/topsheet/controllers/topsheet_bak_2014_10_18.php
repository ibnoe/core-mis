...
class Topsheet extends Front_Controller{
	...
	*/
	...
	private function save_topsheet(){
		...
		if($this->form_validation->run() === TRUE){
			...
			for($i=1; $i<=$no; $i++){
				...
				$par =0;
				// PAR / RISK
				if($this->input->post("data_freq_".$i) == 0 AND $data_id != 0 AND $data_angsuran_pokok !=0 ){ 
				...
				/*if($tabsukarela_saldo < 0){
					$this->session->set_flashdata('message', "success|Tab. Sukarela ".$this->input->post('data_account_'.$i)." < 0 ");
					redirect("topsheet/ts_entry/".$group_id); 
					return false; 
				}*/
			...
			$grand_total = $total_angsuran_pokok+$total_angsuran_profit+$total_tabwajib_debet-$total_tabwajib_credit+$total_tabsukarela_debet-$total_tabsukarela_credit+$total_adm+$total_butab+$total_asuransi+$total_lwk;
			$grand_total_tabungan = $total_tabwajib_debet-$total_tabwajib_credit+$total_tabsukarela_debet-$total_tabsukarela_credit;
	...
	public function download(){			
			...
			$html .= '<tr><td>Akumulasi TR</td><td>: </td></tr>';
			...
			$html .= '	<th colspan="7" class="bdr_btm_bold"><b>Pembiayaan</b></th>';
			$html .= '	<th rowspan="2" class="nobdr" width="2px">&nbsp;</th>';
			...
			if($c->data_status != 4){
				...
				if($c->data_status == 1){
					...
					$angsuranke_sekarang= $c->data_angsuranke + 1;
			...
			$html .= '<tr>';
				...
				$html .= '<td class="nobdr" width="2px">&nbsp;</td>';
				$html .= '<td colspan="6" class="bdr_btm_bold">Sub Total</td>';
			...
			$html .= '<tr>';
				...
				$html .= '<td colspan="2" rowspan="6" class="nobdr" align="center">RF</td>';
			...
			$html .= '<tr>';
				...
				$html .= '<td colspan="18" class="border_bold"><b>TOTAL</b></td>'; 
	...
	public function save_topsheet2(){
		$user_branch = $this->session->userdata('user_branch');	
