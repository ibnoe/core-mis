...
class Ts_daily extends Front_Controller{
	
	private $per_page 	= '10';
	private $title 		= 'ts_daily';
	private $module 	= 'ts_daily';
	...
	*/
	...
	public function tsdaily_apel_download(){
		if($this->session->userdata('logged_in'))
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue("P4", "TOTAL ANGGOTA");
				$objPHPExcel->getActiveSheet()->mergeCells("P4:P5");
			$objPHPExcel->getActiveSheet()->setCellValue("Q4", "TOTAL TR");
				$objPHPExcel->getActiveSheet()->mergeCells("Q4:Q5");
			
			$objPHPExcel->getActiveSheet()->getStyle('A4:Q5')->getAlignment()->setWrapText(true); 
			
			$objPHPExcel->getActiveSheet()->getStyle("K4:Q5")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			...
			foreach($tsdaily as $c):
				...
				if($tgl_start != $tgl_end AND $no==1){
					...
					$objPHPExcel->getActiveSheet()->setCellValue("P$cell_no", $total_transaction);
					$objPHPExcel->getActiveSheet()->setCellValue("Q$cell_no", $total_tr);
				...
				$objPHPExcel->getActiveSheet()->setCellValue("H$cell_no", ($c->tsdaily_total_rf + $c->tsdaily_tabwajib));
				$objPHPExcel->getActiveSheet()->setCellValue("I$cell_no", ($c->tsdaily_total_tabungan - $c->tsdaily_tabwajib));
				...
				$objPHPExcel->getActiveSheet()->setCellValue("O$cell_no", $c->tsdaily_absen_a);
				$objPHPExcel->getActiveSheet()->setCellValue("O$cell_no", $c->tsdaily_absen_a);
				$objPHPExcel->getActiveSheet()->setCellValue("P$cell_no", $c->tsdaily_total_transaction);
				$objPHPExcel->getActiveSheet()->setCellValue("Q$cell_no", $c->tsdaily_total_tr);
				$objPHPExcel->getActiveSheet()->getStyle("C$cell_no:J$cell_no")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
				$objPHPExcel->getActiveSheet()->getStyle("K$cell_no:O$cell_no")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
