...
class finance_week_model extends MY_Model {
	...
	//DESA
	public function count_desa_by_branch($branch)
	{
		if($branch == 0){		
			return $this->db->select("count(DISTINCT client_desa) as numrows")
						->from('tbl_clients')
	...
	public function sum_pembiayaan_aktif($branch)
	{if($branch==0){$branch="";}
		$query= $this->db->query("SELECT SUM(data_plafond) as totals FROM  tbl_pembiayaan JOIN tbl_clients ON tbl_clients.client_id=tbl_pembiayaan.data_client WHERE tbl_clients.client_branch LIKE '%$branch%' AND tbl_pembiayaan.deleted='0' AND tbl_pembiayaan.data_status='1' ");
	...
	public function count_total_pembiayaan_by_branch($branch,$startdate,$enddate)
	{
		if($branch == 0){	
			return $this->db	->select("sum(data_plafond) as numrows")
								...
								->where("tbl_clients.client_branch != '0'")
	...
	//get pangajuan
	public function count_pengajuan($branch,$startdate,$enddate)
	{
		if($branch == 0){		
	...
	public function count_total_pengajuan($branch,$startdate,$enddate)
	{
		if($branch == 0){		
			return $this->db->select("sum(data_plafond) as numrows")
							->from('tbl_pembiayaan')
