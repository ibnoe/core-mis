...
class tabsipeci extends Front_Controller{
	
	private $per_page 	= '15';
	private $title 		= 'tabsipeci';
	private $module 	= 'tabsipeci';
	...
	public function __construct(){
		...
		$this->load->model('tabsipeci_model');
		$this->load->model('tabsipeci_tr_model');
	...
	public function index(){
		...
		{
			redirect('tabsipeci/browse', 'refresh');
		}
		else
		{
	...
	public function browse($page='0')
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$total_rows = $this->tabsipeci_model->count_all($this->input->post('q'),$user_branch);
			...
			$clients = $this->tabsipeci_model->get_all_clients( $config['per_page'] , $page, $this->input->post('q'), $search_key, $user_branch);

			$this->template	->set('menu_title', 'Tabungan Sipeci')
	...
	public function register(){
		...
		if($this->save_tabsipeci()){
			$this->session->set_flashdata('message', 'success|Tabungan Sipeci telah ditambahkan');
		...
		$this->template	->set('menu_title', 'Registrasi Tabungan Sipeci')
						->set('group', $group)
						->set('menu_saving', 'active')
						->build('tabsipeci_register');	
		
	}
	
	private function save_tabsipeci(){
			...
			for($i=1; $i<=$no; $i++){
				...
				if($client_reg == "1"){					
					...
					if($client_id AND $paket AND $waktu AND $this->input->post("client_date_".$i) ){
						...
						//INSERT PEMBIAYAAN
						$data = array(
								'tabsipeci_account'    	=> $client_account,
								'tabsipeci_client'    	=> $this->input->post("client_name_".$i),
								'tabsipeci_date'  		=> $this->input->post("client_date_".$i),
								'tabsipeci_plafond'    	=> $paket,
								'tabsipeci_minggu'    	=> $waktu,
								'tabsipeci_debet'    	=> 0,
								'tabsipeci_credit'    	=> 0,
								'tabsipeci_saldo'    	=> 0							
						);
						
						$check = $this->tabsipeci_model->get_account($client_account);
						$check = $check[0]->client_account;
						
						if(!$check){$this->tabsipeci_model->insert($data);}
						else{ return false;	 }
	...
	public function tabsipeci_view(){
		...
		//GET DETAILS OF TAB Sipeci
		
		$data = $this->tabsipeci_model->get_transaction($client_account);
		//$data = $data[0];
		
		$client = $this->tabsipeci_model->get_account($client_account);		
		$client = $client[0];
 
		$this->template	->set('data', $data)
						->set('menu_title', 'Tabungan Sipeci')
						->set('menu_saving', 'active')
						->set('list', $list)
						->set('no', $no)
						->set('config', $config)
	...
	public function tabsipeci_edit(){
		$client_account =  $this->uri->segment(3);
		$total_rows = $this->tabsipeci_model->count_all_transaction($client_account);
		...
		$data = $this->tabsipeci_model->get_transaction($client_account,$config['per_page'] , $page, $this->input->post('q'));
		//$data = $data[0];
		...
		$this->template	->set('data', $data)
						...
						->set('no', $no)
						->set('config', $config)
						->build('tabsipeci_edit');	
	}
	
	
	public function update_tabsipeci(){
		$user_branch = $this->session->userdata('user_branch');
		$clients = $this->tabsipeci_model->get_all_clients( $config['per_page'] , $page, $this->input->post('q'), $search_key, $user_branch);
		foreach($clients as $c){
			$client_account = $c->tabsipeci_account;
			$plafond = $c->tabsipeci_plafond;
			$tb_id = $c->tabsipeci_id;
			$history = $this->tabsipeci_model->get_transaction2($client_account);	
			echo $tb_id." ".$c->tabsipeci_account." <br/>";
				$saldo=0;
				$no=1;
				...
				foreach($history as $h){
					...
					echo $tr_id.". ".$h->tr_debet." -- ".$saldo_transaksi ." - $minggu<br/>";
						...
						//$this->tabsipeci_tr_model->update($tr_id,$data_tr);
						//$this->tabsipeci_model->update($client_account,$data_tb);
						$no++;
				}
			echo "<br/>";
		}
