...
class target_model extends MY_Model {
	...
	public function get_all($limit, $offset, $branch=''){
		...
		if($branch == 0){$branch = ""; }
		$this->db->join('tbl_branch', 'tbl_branch.branch_id = tbl_target.target_branch', 'left')
	...
	public function get_some_target_per_officer($limit, $offset, $key='', $search_officer='', $search_branch='')
	{
		if($key == 'o')
		{
			return $this->db->select('target_id, target_category, target_item, target_amount, target_startdate, target_enddate, target_remarks, target_officer, target_branch, officer_name, branch_name')
							->from($this->table)
		...
		{
			return $this->db->select('target_id, target_category, target_item, target_amount, target_startdate, target_enddate, target_remarks, officer_name, branch_name')
							->from($this->table)
							->where('tbl_target.deleted','0')
