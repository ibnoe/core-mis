	...
	public function laba_rugi()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			endforeach; 
				...
				$grand_total_pendapatan_saldo = $saldo_awal+$grand_total_pendapatan_debet-$grand_total_pendapatan_credit;
				$grand_total_beban_saldo = $saldo_awal+$grand_total_beban_debet-$grand_total_beban_credit;
	...
	public function neraca_simple()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			foreach($accounting as $c):
				...
				if($haschild == "1" AND $parent == "0"){
					//LEVEL 1
					$account_debet = $this->jurnal_model->sum_account_parent_debet($code_level1);
					$account_credit = $this->jurnal_model->sum_account_parent_credit($code_level1);
					$account_saldo = $saldo_awal + $account_debet - $account_credit;
					...
					//LEVEL 3
					//grand total dihitung dari total account level 3					
					$account_debet = $this->jurnal_model->sum_account_debet($code);
					$account_credit = $this->jurnal_model->sum_account_credit($code);
					$account_saldo = $saldo_awal + $account_debet - $account_credit;					
		...
		if($this->session->userdata('logged_in'))
		{
			...
			foreach($accounting as $c):
				...
				if($haschild == "1" AND $parent == "0"){
					...
					//LEVEL 2					
					$account_debet = $this->jurnal_model->sum_account_parent_debet($code_level2);
					$account_credit = $this->jurnal_model->sum_account_parent_credit($code_level2);
					$account_saldo = $saldo_awal + $account_debet - $account_credit;
					$neraca .= '<tr>     
								<td align="left" >&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.site_url($this->module.'/general_ledger_detail/'.$c->accounting_code).'" title="view details"><b>'.$c->accounting_code." ".$c->accounting_name.'</b></a></td>	
								<td class="text-right"><b>0</b></td>
