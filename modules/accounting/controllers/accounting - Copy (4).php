...
class Accounting extends Front_Controller{
	...
	public function jurnal($page='0')
	{
		if($this->session->userdata('logged_in'))
		{
			...
			//Get Jurnal
			$total_rows = $this->jurnal_model->count_all_jurnal($this->input->post('q'),$this->input->post('key'),$user_branch);
			$jurnal = $this->jurnal_model->get_all_jurnal($config['per_page'] ,$page,$this->input->post('q'),$this->input->post('key'),$user_branch);
	...
	public function neraca()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$get_neraca = $this->print_neraca("ASET",$accounting,$date_start,$date_end,$date_start_before,$date_end_before);
			...
			$get_neraca = $this->print_neraca("KEWAJIBAN",$accounting,$date_start,$date_end,$date_start_before,$date_end_before);
			list($neraca_list, $aset_before, $aset_debet, $aset_credit, $aset_saldo) = $get_neraca;			
			$neraca .= $neraca_list;
	...
	private function print_neraca($label,$accounting,$date_start,$date_end,$date_start_before,$date_end_before){
		foreach($accounting as $c):
				$code = $c->accounting_code;
				$code_level0 = substr($code,0,1);
				$code_level1 = substr($code,0,3);
